...
class AdaptiveLearningEngineV9 {
  ...
  static Future<List<String>> weakSkills(String stageId) async {
    ...
    final weak=all.values.where((x)=>x.needsReview).toList()..sort((a,b)=>a.mastery.compareTo(b.mastery));
  ...
  static Future<List<LearningRecommendationV9>> recommendations(CurriculumStageV8 stage, Set<String> completedUnits) async {
    ...
    final allSkills=await skills(stage.id);
    final weak=allSkills.values.where((x)=>x.needsReview).toList()..sort((a,b)=>a.mastery.compareTo(b.mastery));
    for(final s in weak.take(3)) {
      out.add(LearningRecommendationV9(type:'review',title:'مراجعة ${s.skillId}',reason:'هذه المهارة تحتاج تدريباً إضافياً',icon:'🔄',skill:s.skillId));
    }
