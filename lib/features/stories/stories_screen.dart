      ...
      child: Scaffold(
        ...
        body: ListView(
          ...
          children: [
            ...
            for (final s in storiesV11)
              Card(
                child: ListTile(
                  leading: Text(s.emoji, style: const TextStyle(fontSize: 38)),
                  title: Text(s.title, style: const TextStyle(fontWeight: FontWeight.bold)),
                  subtitle: Text('${s.stage} • ${s.text}', maxLines: 2, overflow: TextOverflow.ellipsis),
                  onTap: () => Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (_) => StoryPage(s: s.title, text: s.text, emoji: s.emoji),
                    ),
                  ),
                ),
              ),
