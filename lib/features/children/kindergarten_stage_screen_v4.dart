    ...
    return Directionality(
      ...
      child: Scaffold(
        ...
        body: ListView(padding: const EdgeInsets.all(16), children: [
          ...
          for (final item in items) Padding(padding: const EdgeInsets.only(bottom: 12), child: _MenuCard(title: item.title, subtitle: item.subtitle, icon: item.icon, color: item.color, onTap: () => _open(context, item.title))),
        ]),
      ),
    );
  }
...
class _KgItem { final String title, subtitle; final IconData icon; final Color color; const _KgItem(this.title, this.subtitle, this.icon, this.color); }
class _MenuCard extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => App3DCard(onTap: onTap, encouragement: '✨ $title', child: Container(padding: const EdgeInsets.all(17), decoration: BoxDecoration(gradient: LinearGradient(colors: [color, color.withValues(alpha: 0.68)]), borderRadius: BorderRadius.circular(22)), child: Row(children: [Icon(icon, color: Colors.white, size: 42), const SizedBox(width: 13), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: const TextStyle(color: Colors.white, fontSize: 21, fontWeight: FontWeight.w900)), Text(subtitle, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w600))])), const Icon(Icons.arrow_back_ios_new_rounded, color: Colors.white)])));
}
...
class _Btn extends StatelessWidget {
  final String text; final Color color; final VoidCallback onTap; final bool selected;
  const _Btn(this.text, this.color, this.onTap, {this.selected = false});
  @override Widget build(BuildContext context) => AnimatedContainer(duration: const Duration(milliseconds: 180), transform: selected ? (Matrix4.identity()..scaleByDouble(1.02, 1.02, 1.0, 1.0)) : Matrix4.identity(), child: App3DCard(onTap: onTap, encouragement: '✨ $text', child: Container(padding: const EdgeInsets.symmetric(vertical: 14, horizontal: 8), decoration: BoxDecoration(color: selected ? color.withValues(alpha: 0.9) : color, borderRadius: BorderRadius.circular(18), border: selected ? Border.all(color: Colors.white, width: 3) : null), child: Center(child: Text(text, textAlign: TextAlign.center, style: const TextStyle(color: Colors.white, fontSize: 17, fontWeight: FontWeight.w900))))));
}
class _LettersPage extends StatefulWidget { final bool kg2; const _LettersPage({required this.kg2}); @override State<_LettersPage> createState() => _LettersPageState(); }
class _LettersPageState extends State<_LettersPage> {
  int index = 0; int form = 0;
  static const forms = <String, List<String>>{
    'ا': ['ا', 'ـا', 'ـا'], 'ب': ['بـ', 'ـبـ', 'ـب'], 'ت': ['تـ', 'ـتـ', 'ـت'], 'ث': ['ثـ', 'ـثـ', 'ـث'], 'ج': ['جـ', 'ـجـ', 'ـج'], 'ح': ['حـ', 'ـحـ', 'ـح'], 'خ': ['خـ', 'ـخـ', 'ـخ'], 'د': ['د', 'ـد', 'ـد'], 'ذ': ['ذ', 'ـذ', 'ـذ'], 'ر': ['ر', 'ـر', 'ـر'], 'ز': ['ز', 'ـز', 'ـز'], 'س': ['سـ', 'ـسـ', 'ـس'], 'ش': ['شـ', 'ـشـ', 'ـش'], 'ص': ['صـ', 'ـصـ', 'ـص'], 'ض': ['ضـ', 'ـضـ', 'ـض'], 'ط': ['طـ', 'ـطـ', 'ـط'], 'ظ': ['ظـ', 'ـظـ', 'ـظ'], 'ع': ['عـ', 'ـعـ', 'ـع'], 'غ': ['غـ', 'ـغـ', 'ـغ'], 'ف': ['فـ', 'ـفـ', 'ـف'], 'ق': ['قـ', 'ـقـ', 'ـق'], 'ك': ['كـ', 'ـكـ', 'ـك'], 'ل': ['لـ', 'ـلـ', 'ـل'], 'م': ['مـ', 'ـمـ', 'ـم'], 'ن': ['نـ', 'ـنـ', 'ـن'], 'ه': ['هـ', 'ـهـ', 'ـه'], 'و': ['و', 'ـو', 'ـو'], 'ي': ['يـ', 'ـيـ', 'ـي'],
  };
  @override Widget build(BuildContext context) {
    final x = arabicLetters[index]; final f = forms[x.letter] ?? [x.letter, x.letter, x.letter]; final labels = ['أولي', 'وسطي', 'آخري'];
    return _Page('الحروف العربية', [Text('${arNum(index + 1)} من ${arNum(28)}', textAlign: TextAlign.center, style: const TextStyle(fontWeight: FontWeight.w900)), const SizedBox(height: 10), App3DCard(onTap: () => VoiceService.arabicLetterSound(x.letter, fallbackText: x.sound), encouragement: '🔊 صوت الحرف', child: Container(padding: const EdgeInsets.all(18), decoration: const BoxDecoration(gradient: LinearGradient(colors: [Color(0xFF7652FF), Color(0xFF536DFE)]), borderRadius: BorderRadius.all(Radius.circular(24))), child: Column(children: [Text(widget.kg2 ? f[form] : x.letter, style: const TextStyle(color: Colors.white, fontSize: 105, fontWeight: FontWeight.w900)), Text('صوت القراءة: ${x.sound}', style: const TextStyle(color: Colors.white, fontSize: 19, fontWeight: FontWeight.w700)), const SizedBox(height: 4), Text('اسم الحرف: ${x.letter}', style: const TextStyle(color: Colors.white, fontSize: 17, fontWeight: FontWeight.w700))]))), if (widget.kg2) ...[const SizedBox(height: 12), Row(children: [for (var j = 0; j < 3; j++) Expanded(child: Padding(padding: EdgeInsets.only(left: j == 0 ? 0 : 5), child: _Btn(labels[j] + (form == j ? ' ✓' : ''), const Color(0xFF7652FF), () => setState(() => form = j), selected: form == j)))]), const SizedBox(height: 8), Text('اختر شكل الحرف ثم اسمع النطق واكتبه على السبورة.', textAlign: TextAlign.center, style: TextStyle(color: Colors.grey.shade700, fontWeight: FontWeight.w700))], const SizedBox(height: 12), Row(children: [Expanded(child: _Btn('السابق', const Color(0xFFE94F9B), index > 0 ? () => setState(() { index--; form = 0; }) : () => AppFeedback.show('💛 أنت في البداية'))), const SizedBox(width: 7), Expanded(child: _Btn('🔊 نطق', const Color(0xFF18A7E8), () => VoiceService.arabicLetterSound(x.letter, fallbackText: x.sound))), const SizedBox(width: 7), Expanded(child: _Btn('التالي', const Color(0xFF16B878), index < 27 ? () => setState(() { index++; form = 0; }) : () => AppFeedback.show('🏆 أكملت الحروف!')))])]);
  }
}
class _NumbersPage extends StatefulWidget { final bool kg2; const _NumbersPage({required this.kg2}); @override State<_NumbersPage> createState() => _NumbersPageState(); }
...
class _PlaceValuePageState extends State<_PlaceValuePage> {
  ...
  @override Widget build(BuildContext context) {
    ...
    return _Page('الآحاد والعشرات', [const Text('نميز رقم الآحاد عن رقم العشرات بصريًا وتعليميًا.', textAlign: TextAlign.center, style: TextStyle(fontSize: 18, fontWeight: FontWeight.w900)), const SizedBox(height: 10), Text(arNum(number), textAlign: TextAlign.center, style: const TextStyle(fontSize: 70, fontWeight: FontWeight.w900)), const SizedBox(height: 6), Row(children: [Expanded(child: _ValueCard('العشرات', tens, const Color(0xFF7652FF), selected == 1, () => setState(() => selected = 1))), const SizedBox(width: 10), Expanded(child: _ValueCard('الآحاد', ones, const Color(0xFF16B878), selected == 2, () => setState(() => selected = 2)))]), const SizedBox(height: 10), Text(selected == 1 ? 'العشرات: ${arNum(tens)} عشرات = ${arNum(tens * 10)}' : selected == 2 ? 'الآحاد: ${arNum(ones)} آحاد' : 'اضغط على الآحاد أو العشرات للتوضيح', textAlign: TextAlign.center, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w800)), Slider(min: 10, max: 50, divisions: 40, value: number.toDouble(), label: arNum(number), onChanged: (v) => setState(() { number = v.round(); selected = 0; }))]);
  }
}
class _ValueCard extends StatelessWidget { final String title; final int value; final Color color; final bool selected; final VoidCallback onTap; const _ValueCard(this.title, this.value, this.color, this.selected, this.onTap); @override Widget build(BuildContext context) => App3DCard(onTap: onTap, encouragement: title, child: Container(padding: const EdgeInsets.all(15), decoration: BoxDecoration(color: color, borderRadius: BorderRadius.circular(18), border: selected ? Border.all(color: Colors.white, width: 4) : null), child: Column(children: [Text(title, style: const TextStyle(color: Colors.white, fontSize: 19, fontWeight: FontWeight.w900)), Text(arNum(value), style: const TextStyle(color: Colors.white, fontSize: 42, fontWeight: FontWeight.w900))]))); }
class _WritingPage extends StatefulWidget { final bool kg2; const _WritingPage({required this.kg2}); @override State<_WritingPage> createState() => _WritingPageState(); }
class _WritingPageState extends State<_WritingPage> {
  ...
  @override Widget build(BuildContext context) {
    final letters = tab == 0; final max = words ? words2.length : (letters ? 28 : (widget.kg2 ? 50 : 10)); final guide = words ? words2[index] : letters ? arabicLetters[index].letter : arNum(index + 1);
    return _Page('الكتابة', [Row(children: [Expanded(child: _Btn('الحروف', const Color(0xFF7652FF), () => setState(() { tab = 0; index = 0; words = false; }), selected: letters && !words)), const SizedBox(width: 8), Expanded(child: _Btn('الأرقام', const Color(0xFF18A7E8), () => setState(() { tab = 1; index = 0; words = false; }), selected: !letters && !words))]), if (widget.kg2) ...[const SizedBox(height: 8), _Btn('🔤 كلمات من حرفين', const Color(0xFFFF8A3D), () => setState(() { words = true; index = 0; }), selected: words)], const SizedBox(height: 12), Text(words ? 'اكتب الكلمة:' : 'اكتب:', textAlign: TextAlign.center, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w800)), Text(guide, textAlign: TextAlign.center, style: const TextStyle(fontSize: 58, fontWeight: FontWeight.w900)), const SizedBox(height: 4), _DrawingBoard(color: ink, onColorChanged: (c) => setState(() => ink = c)), const SizedBox(height: 8), Row(children: [Expanded(child: _Btn('السابق', const Color(0xFFE94F9B), index > 0 ? () => setState(() => index--) : () => AppFeedback.show('💛 البداية'))), const SizedBox(width: 8), Expanded(child: _Btn('🔊 اسمع', const Color(0xFF7652FF), () => words ? VoiceService.arabic(words2[index]) : letters ? VoiceService.arabicLetterSound(arabicLetters[index].letter, fallbackText: arabicLetters[index].sound) : VoiceService.arabic(_numberName(index + 1)))), const SizedBox(width: 8), Expanded(child: _Btn('التالي', const Color(0xFF16B878), index < max - 1 ? () => setState(() => index++) : () => AppFeedback.show('🏆 أحسنت!')))]);
  }
}
class _DrawingBoard extends StatefulWidget { final Color color; final ValueChanged<Color> onColorChanged; const _DrawingBoard({required this.color, required this.onColorChanged}); @override State<_DrawingBoard> createState() => _DrawingBoardState(); }
class _DrawingBoardState extends State<_DrawingBoard> {
  ...
  @override Widget build(BuildContext context) {
    ...
    return Column(children: [Container(height: 290, decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(24), border: Border.all(color: widget.color, width: 3)), child: Listener(behavior: HitTestBehavior.opaque, onPointerDown: (e) => setState(() => current = <Offset>[e.localPosition]), onPointerMove: (e) => setState(() => current = <Offset>[...current, e.localPosition]), onPointerUp: (_) { if (current.isNotEmpty) strokes.add(List<Offset>.of(current)); setState(() => current = <Offset>[]); }, onPointerCancel: (_) => setState(() => current = <Offset>[]), child: CustomPaint(painter: _DrawingPainter(widget.color, strokes, current), child: const SizedBox.expand()))), const SizedBox(height: 6), SingleChildScrollView(scrollDirection: Axis.horizontal, child: Row(children: [const Text('لون القلم: ', style: TextStyle(fontWeight: FontWeight.w800)), for (final color in colors) GestureDetector(onTap: () => widget.onColorChanged(color), child: Container(width: 34, height: 34, margin: const EdgeInsets.all(4), decoration: BoxDecoration(color: color, shape: BoxShape.circle, border: Border.all(color: Colors.black12))))])), const SizedBox(height: 6), Row(children: [Expanded(child: _Btn('↩ تراجع', const Color(0xFF7652FF), undo)), const SizedBox(width: 8), Expanded(flex: 2, child: _Btn('🗑 مسح السبورة', const Color(0xFFE53935), clear))])]);
  }
}
class _DrawingPainter extends CustomPainter { final Color color; final List<List<Offset>> strokes; final List<Offset> current; _DrawingPainter(this.color, this.strokes, this.current); @override void paint(Canvas canvas, Size size) { final paint = Paint()..color = color..strokeWidth = 11..strokeCap = StrokeCap.round..strokeJoin = StrokeJoin.round; void drawStroke(List<Offset> points) { for (var i = 1; i < points.length; i++) { canvas.drawLine(points[i - 1], points[i], paint); } } for (final stroke in strokes) { drawStroke(stroke); } drawStroke(current); } @override bool shouldRepaint(covariant _DrawingPainter oldDelegate) => true; }
...
class _GamesPage extends StatelessWidget { final bool kg2; const _GamesPage({required this.kg2}); @override Widget build(BuildContext context)=>_Page(kg2?'القصص والألعاب':'الألعاب',[if(kg2)_GameButton(title:'📖 قصة الأرنب المجتهد',color:const Color(0xFF7652FF),child:const _StoryPage(title:'الأرنب المجتهد',pages:['كان أرنب صغير يحب التعلم.','تعلم الحروف ثم الأرقام كل يوم.','فرح لأنه لم يستسلم وأصبح مجتهدًا!'])),if(kg2)_GameButton(title:'📖 قصة رحلة الحروف',color:const Color(0xFF18A7E8),child:const _StoryPage(title:'رحلة الحروف',pages:['خرجت الحروف في رحلة جميلة.','التقت كل مجموعة بحرف جديد.','عاد الجميع وهم يعرفون أشكال الحروف!'])),_GameButton(title:'🔤 لعبة الحروف',color:const Color(0xFFE94F9B),child:_LetterGame()),if(kg2)_GameButton(title:'🔗 لعبة دمج الحروف',color:const Color(0xFFFF8A3D),child:_MergeGame()),_GameButton(title:'🔢 لعبة الأرقام',color:const Color(0xFF16B878),child:_NumberGame(max:kg2?20:10)),if(kg2)_GameButton(title:'🔟 لعبة العشرات',color:const Color(0xFF7652FF),child:const _PlaceGame())]); }
