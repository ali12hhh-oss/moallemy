...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget _childCard() {
    ...
    return _PressCard(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(gradient: LinearGradient(colors: [c.primaryContainer, c.secondaryContainer]), borderRadius: BorderRadius.circular(24)),
        child: Row(children: [
          Container(width: 62, height: 62, decoration: BoxDecoration(color: c.surface.withValues(alpha: .75), shape: BoxShape.circle), child: const Center(child: Text('🧒', style: TextStyle(fontSize: 34)))),
          const SizedBox(width: 14),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(child == null ? 'اكتب اسمك يا بطل ⭐' : 'أهلاً يا ${child!.name} 🌟', style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w900)),
            const SizedBox(height: 5),
            Text(child == null ? 'سجّل اسمك ومرحلتك لنحفظ تقدمك.' : 'المرحلة: ${child!.stage}  •  اضغط لتعديل البيانات', style: TextStyle(color: c.onSurfaceVariant)),
          ])),
  ...
  Widget _parentCard() => _PressCard(
    onTap: _openParents,
    child: ListTile(contentPadding: const EdgeInsets.all(16), leading: const CircleAvatar(radius: 28, child: Text('👨‍👩‍👧', style: TextStyle(fontSize: 25))), title: const Text('متابعة الأسرة', style: TextStyle(fontSize: 21, fontWeight: FontWeight.w900)), subtitle: const Text('نتائج الطفل، المهارات المتقنة، ما يحتاج إلى مراجعة والإنجازات'), trailing: const Icon(Icons.arrow_back_ios_new_rounded)),
  );
  ...
  List<Widget> _stageCards() {
    const stages = [
      ...
      ('prep', 'التمهيدي', '٥–٦ سنوات', '📚', 'حركات وتهجي وقراءة وأعداد وتمهيد إنجليزي'),
    ...
    return stages.map((s) => Padding(padding: const EdgeInsets.only(bottom: 10), child: _PressCard(onTap: () => _openStage(s.$1), child: ListTile(contentPadding: const EdgeInsets.symmetric(horizontal: 18, vertical: 9), leading: Container(width: 54, height: 54, decoration: BoxDecoration(color: Theme.of(context).colorScheme.primaryContainer, borderRadius: BorderRadius.circular(16)), child: Center(child: Text(s.$3.isEmpty ? s.$4 : s.$4, style: const TextStyle(fontSize: 29))),), title: Text(s.$2, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w900)), subtitle: Text('${s.$3} • ${s.$5}'), trailing: const Icon(Icons.arrow_back_ios_new_rounded))))).toList();
  }

  Widget _storeCard() => _PressCard(onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const ShopScreen())), child: ListTile(contentPadding: const EdgeInsets.all(16), leading: const CircleAvatar(radius: 27, child: Text('⭐', style: TextStyle(fontSize: 25))), title: const Text('المتجر', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900)), subtitle: const Text('جوائز وملصقات وشخصيات تشجيعية بالنجوم'), trailing: const Icon(Icons.arrow_back_ios_new_rounded)));
}
...
class _PressCardState extends State<_PressCard> {
  bool pressed = false;
  @override Widget build(BuildContext context) => AnimatedScale(scale: pressed ? .975 : 1, duration: const Duration(milliseconds: 100), child: Card(child: InkWell(onTap: widget.onTap, onHighlightChanged: (v) => setState(() => pressed = v), borderRadius: BorderRadius.circular(24), child: widget.child)));
}
