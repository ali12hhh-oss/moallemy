...
class FinalExamScreenV8 extends StatefulWidget {
  ...
  @override
  State<FinalExamScreenV8> createState() => _FinalExamScreenV8State();
}
...
class _FinalExamScreenV8State extends State<FinalExamScreenV8> {
  ...
  void initState() {
    super.initState();
    questions = widget.stage.id == 'prep'
        ? _prepQuestions()
        : _questions(widget.stage.id);
  }
                ...
                child: Padding(
                  ...
                  child: Column(
                    children: [
                      ...
                      if (q.voiceText != null)
                        IconButton(
                          onPressed: () => VoiceService.arabic(q.voiceText!),
  ...
  Widget _prepBody(_Question q) {
    switch (q.type) {
      case _QuestionType.write:
        return Column(
          children: [
            ...
            _ExamDrawingBoard(onDone: _nextPrep),
          ],
        );
      case _QuestionType.place:
        ...
        final target = q.target == 'ones' ? ones : tens;
        return Column(
          children: [
            Text(
              arNum(number),
            ...
            Text(
              q.target == 'ones' ? 'اضغط رقم الآحاد' : 'اضغط رقم العشرات',
              textAlign: TextAlign.center,
            ...
            Row(
              ...
              children: [
                _DigitChoice(number ~/ 10, target, _nextPrep),
      ...
      case _QuestionType.shape:
        return Wrap(
          ...
          spacing: 10,
          runSpacing: 10,
          children: q.shapeOptions!
              .map(
                (name) => _ShapeChoice(
                  name: name,
                  correct: name == q.answer,
                  onTap: _nextPrep,
                ),
              )
              .toList(),
      ...
      case _QuestionType.color:
        return Wrap(
          ...
          spacing: 12,
          runSpacing: 12,
          children: q.colorOptions!
              .asMap()
              .entries
              .map(
                (e) => _ColorChoice(
                  color: e.value,
                  correct: e.key == q.colorAnswer,
                  onTap: _nextPrep,
                ),
              )
              .toList(),
  ...
  Widget _result() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      await ProgressV8.recordFinalExam(
        widget.stage.id,
        score,
        questions.length,
        passed,
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  passed
                      ? 'أحسنت! اجتزت المرحلة'
                      : 'أحسنت المحاولة! نراجع ثم نعيد الاختبار',
                  textAlign: TextAlign.center,
                  style: const TextStyle(
                    fontSize: 25,
                    fontWeight: FontWeight.bold,
                ...
                const SizedBox(height: 12),
                Text(
                  'نتيجتك: ${arNum(score)} من ${arNum(questions.length)}',
                  style: const TextStyle(fontSize: 23),
                ),
                Text(
                  'النسبة: ${arNum((score / questions.length * 100).round())}٪',
                  style: const TextStyle(fontSize: 23),
                ),
                const SizedBox(height: 20),
                Text(
                  passed
                      ? '🏆 حصلت على ٥٠ نجمة و٥٠٠ نقطة تعليمية'
                      : '🔄 راجع المهارات الضعيفة ثم حاول مرة أخرى',
                  textAlign: TextAlign.center,
                ),
                const SizedBox(height: 20),
                FilledButton(
                  onPressed: () => Navigator.pop(context),
                  child: const Text('العودة إلى المنهج'),
                ),
              ],
            ),
          ),
  ...
  List<_Question> _prepQuestions() => [
    ...
    const _Question.shape('اختر شكل المثلث', 'مثلث', [
      'مربع',
      'مثلث',
      'دائرة',
      'مستطيل',
    ]),
    const _Question.shape('اختر شكل المستطيل', 'مستطيل', [
      'دائرة',
      'مثلث',
      'مستطيل',
      'مربع',
    ]),
    _Question.color('اختر اللون الأحمر', const [
      Color(0xFF2196F3),
      Color(0xFFF44336),
      Color(0xFF4CAF50),
      Color(0xFFFFC107),
    ], 1),
    _Question.color('اختر اللون الأزرق', const [
      Color(0xFFFFC107),
      Color(0xFF4CAF50),
      Color(0xFF2196F3),
      Color(0xFFF44336),
  ...
  List<_Question> _questions(String id) {
    ...
    if (id == 'g2')
      return [
        ...
        const _Question(
          'أي جملة صحيحة؟',
          ['الولد يقرأ.', 'يقرأ الولد؟', 'الولد قراءة.', 'قراءة الولد هو.'],
          'الولد يقرأ.',
          '📖',
          null,
        ),
    ...
    return [
      ...common,
      const _Question(
        'اختر الكلمة المقسمة إلى مقطعين',
        ['مَدْرَسَة', 'باب', 'قلم', 'نور'],
        'مَدْرَسَة',
        '📚',
        null,
      ),
...
class _Question {
  ...
  const _Question(
    ...
    this.emoji,
    this.voiceText,
  ) : type = _QuestionType.normal,
      target = null,
      number = null,
      shapeOptions = null,
      colorOptions = null,
      colorAnswer = null;
  const _Question.write(this.prompt, this.target, this.voiceText)
    : options = const [],
      answer = '',
      emoji = '✍️',
      type = _QuestionType.write,
      number = null,
      shapeOptions = null,
      colorOptions = null,
      colorAnswer = null;
  const _Question.place(this.prompt, this.number, this.target)
    : options = const [],
      answer = '',
      emoji = '🔢',
      voiceText = null,
      type = _QuestionType.place,
      shapeOptions = null,
      colorOptions = null,
      colorAnswer = null;
  const _Question.shape(this.prompt, this.answer, this.shapeOptions)
    : options = const [],
      emoji = '🔷',
      voiceText = null,
      type = _QuestionType.shape,
      target = null,
      number = null,
      colorOptions = null,
      colorAnswer = null;
  const _Question.color(this.prompt, this.colorOptions, this.colorAnswer)
    : options = const [],
      answer = '',
      emoji = '🎨',
      voiceText = null,
      type = _QuestionType.color,
      target = null,
      number = null,
      shapeOptions = null;
}
...
class _ExamDrawingBoard extends StatefulWidget {
  ...
  @override
  State<_ExamDrawingBoard> createState() => _ExamDrawingBoardState();
}
    ...
    children: [
      Container(
        ...
        child: Listener(
          ...
          onPointerMove: (e) =>
              setState(() => current = [...current, e.localPosition]),
          onPointerUp: (_) {
            if (current.isNotEmpty) strokes.add(List<Offset>.of(current));
            setState(() => current = []);
          },
          child: CustomPaint(
            painter: _ExamPainter(strokes, current),
...
class _ExamPainter extends CustomPainter {
  ...
  _ExamPainter(this.strokes, this.current);
  @override
  void paint(Canvas c, Size s) {
    ...
    void d(List<Offset> a) {
      for (var i = 1; i < a.length; i++) c.drawLine(a[i - 1], a[i], p);
    }

    for (final x in strokes) d(x);
    d(current);
  }

  @override
  bool shouldRepaint(covariant _ExamPainter oldDelegate) => true;
}

class _DigitChoice extends StatelessWidget {
  final int value, correct;
  final void Function({bool correct}) onTap;
  ...
  Widget build(BuildContext context) => SizedBox(
    ...
    height: 90,
    child: FilledButton(
      onPressed: () => onTap(correct: value == correct),
      child: Text(
        arNum(value),
...
class _ShapeChoice extends StatelessWidget {
  ...
  Widget build(BuildContext context) => GestureDetector(
    ...
    child: Container(
      ...
      child: CustomPaint(
        painter: _ShapeOptionPainter(name),
        child: const SizedBox.expand(),
      ),
    ),
  );
...
class _ShapeOptionPainter extends CustomPainter {
  ...
  void paint(Canvas c, Size s) {
    ...
    if (name == 'دائرة') {
      c.drawCircle(center, 38, p);
      c.drawCircle(center, 38, o);
    } else if (name == 'مربع') {
      final r = Rect.fromCenter(center: center, width: 70, height: 70);
      c.drawRect(r, p);
      c.drawRect(r, o);
    } else if (name == 'مستطيل') {
      final r = Rect.fromCenter(center: center, width: 100, height: 60);
      c.drawRect(r, p);
      c.drawRect(r, o);
    } else {
      final path = Path();
      final sides = name == 'مثلث' ? 3 : 4;
      for (var i = 0; i < sides; i++) {
        final a = -1.5708 + i * 6.2832 / sides;
        final pt = center + Offset(42 * math.cos(a), 42 * math.sin(a));
        if (i == 0)
  ...
  @override
  bool shouldRepaint(covariant _ShapeOptionPainter oldDelegate) =>
