...
class EnglishHomeScreen extends StatelessWidget {
  ...
  @override Widget build(BuildContext c) {
    return Scaffold(appBar: AppBar(title: const Text('English Learning')), body: ListView(padding: const EdgeInsets.all(16), children: [
      const Card(child: Padding(padding: EdgeInsets.all(18), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('English', style: TextStyle(fontSize: 30, fontWeight: FontWeight.bold)), Text('For Grade One and above')]))),
...
class EnglishLettersScreen extends StatefulWidget { const EnglishLettersScreen({super.key}); @override State<EnglishLettersScreen> createState()=>_EnglishLettersState(); }
class _EnglishLettersState extends State<EnglishLettersScreen> {
  int i=0;
  @override Widget build(BuildContext c) { final x=englishLetters[i]; return Scaffold(appBar:AppBar(title:const Text('English Phonics')),body:Padding(padding:const EdgeInsets.all(18),child:Column(children:[
    LinearProgressIndicator(value:(i+1)/englishLetters.length), const SizedBox(height:18),
    Text('${x.letter} ${x.letter.toLowerCase()}',style:const TextStyle(fontSize:90,fontWeight:FontWeight.bold)),
    Text('Sound: ${x.sound}',style:const TextStyle(fontSize:25)), Text('${x.emoji}  ${x.word}',style:const TextStyle(fontSize:30)),
