...
class _ELS extends State<EnglishLettersScreen>{int i=0;@override Widget build(BuildContext c){final x=englishLetters[i];return Scaffold(appBar:AppBar(title:const Text('English Phonics')),body:Padding(padding:const EdgeInsets.all(18),child:Column(children:[
 LinearProgressIndicator(value:(i+1)/englishLetters.length),const SizedBox(height:18),Text(x.letter,style:const TextStyle(fontSize:110,fontWeight:FontWeight.bold)),Text('Sound: ${x.sound}',style:const TextStyle(fontSize:25)),Text('${x.emoji}  ${x.word}',style:const TextStyle(fontSize:30)),
 ...
 Row(children:[Expanded(child:OutlinedButton(onPressed:i==0?null:()=>setState(()=>i--),child:const Text('Previous'))),const SizedBox(width:10),Expanded(child:FilledButton(onPressed:i==25?null:()=>setState(()=>i++),child:const Text('Next')))])
]));}}
